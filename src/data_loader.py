...
def load_player_positions(force_download: bool = False, timeout: int = 120) -> pd.DataFrame:
    ...
    DataFrame of [player_id, player_name, position, position_group] keyed by
    `gsis_id`, which is the same ID space as `rusher_player_id` in the
    play-by-play data.
    """
    ...
    return players.rename(columns={"gsis_id": "player_id", "display_name": "player_name"})[
